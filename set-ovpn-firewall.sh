...
# Basic, hardened iptables + ip6tables firewall script for an OpenVPN 2.3.x server running dnscrypt-proxy and unbound for DNS. Designed with security testing in mind. 
# Supports dual-stack IPv4 + IPv6. Hardens against many common attacks, and implements strict filtering of DHCP, ICMPv6, and DHCPv6. 
# Tested on Arch Linux; most Linux should work, also. 
#
...
$IP4TABLES -A ICMP_FORWARD -p icmp -m icmp --icmp-type destination-unreachable -j ACCEPT #Type 3
$IP4TABLES -A ICMP_FORWARD -p icmp -m icmp --icmp-type fragmentation-needed -j ACCEPT #Type 3 Code 4
...
$IP4TABLES -A ICMP_INPUT -p icmp -m icmp --icmp-type echo-reply -m state --state ESTABLISHED,RELATED -j ACCEPT
$IP4TABLES -A ICMP_INPUT -p icmp -m icmp --icmp-type destination-unreachable -j ACCEPT #Type 3
$IP4TABLES -A ICMP_INPUT -p icmp -m icmp --icmp-type fragmentation-needed -j ACCEPT #Type 3 Code 4
...
echo ' (harden) drop ident/auth'
$IP4TABLES -A INPUT -p tcp --dport 113 -m state --state NEW -j DROP
$IP4TABLES -A FORWARD -p tcp --dport 113 -m state --state NEW -j DROP
...
echo ' (harden) drop ident/auth'
$IP6TABLES -A INPUT -p tcp --dport 113 -m state --state NEW -j DROP
$IP6TABLES -A FORWARD -p tcp --dport 113 -m state --state NEW -j DROP
